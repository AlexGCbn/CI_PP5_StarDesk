...
class TestViews(TestCase):
    ...
    def test_product_reviewed(self):
        ...
        response = client.get('/products/case/1/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_admin_add_product_get(self):
        """Test admin add product GET"""
        normal_user = User.objects.create(username='testuser', email='test@email.com')
        normal_user.set_password('test_password')
        normal_user.save()
        admin = User.objects.create_superuser(username='testadmin', email='test@email.com')
